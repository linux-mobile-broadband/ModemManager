...
static void
mbm_erinfo_received (MMSerialPort *port,
                     GMatchInfo *info,
                     gpointer user_data)
{
                 ...
                 gpointer user_data)
{
    ...
    mm_serial_port_queue_command (port, "+CMER=3,0,0,1", 3, NULL, NULL);
    mm_serial_port_queue_command (port, "*ERINFO=1", 3, NULL, NULL);
...
static void
disable_unsolicited_done (MMSerialPort *port,
                          GString *response,
                          GError *error,
                          gpointer user_data)
         ...
         gpointer user_data)
{
    ...
    mm_serial_port_queue_command (primary, "+CMER=0;*ERINFO=0", 5, disable_unsolicited_done, info);
}
           ...
           GError **error)
{
    ...
    if (port && MM_IS_SERIAL_PORT (port)) {
        ...
        regex = g_regex_new ("\\r\\n\\*ERINFO:\\s*(\\d),(\\d),(\\d).*\\r\\n", G_REGEX_RAW | G_REGEX_OPTIMIZE, 0, NULL);
        mm_serial_port_add_unsolicited_msg_handler (MM_SERIAL_PORT (port), regex, mbm_erinfo_received, modem, NULL);
        g_regex_unref (regex);
    }
